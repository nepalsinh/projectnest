...
import { MdAccessTimeFilled, MdGroup, MdManageAccounts } from 'react-icons/md'

const Hero = () => {
    return <>
        <section className="flex max-md:flex-col my-16  md:px-32 px-8">
            <div className="flex md:flex-1 flex-col p-2 gap-4">
                <h1 className="lg:text-6xl md:text-5xl sm:text-4xl text-3xl font-bold text-start  lg:leading-tight">Welcome to <span className="text-violet-400">ProjectNest</span>  Where Ideas Take Flight!</h1>
            ...
            <div className="md:flex-1 ">
                <Tilt tiltMaxAngleX={10} tiltMaxAngleY={10}>
                    <div className="px-10">
                        <img src="/hero.png" alt="PeojectNest" className="" />
                    </div>
                </Tilt>
            </div>
        </section>
...
const Features = () => {
    const features = [
        ...
        {
            title: "Collaboration",
            icon: <MdGroup className="flex-shrink-0 w-7 h-7 text-white" />
        },
        {
            ...
            <div className='grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6'>
                {
                    features.map((feature, index) => {
                        return (
                            <Tilt key={index} scale={1.05} tiltMaxAngleX={10} tiltMaxAngleY={10}>
                                <div onMouseMove={effect}
                                    onMouseLeave={(e) => {
                                    ...
                                    <div className='flex items-center justify-center w-12 h-12 bg-violet-600 rounded-lg mx-auto' >
                                        {feature.icon}
                                    </div>
                                    <div className='mt-3'>
...
function LandingPage() {
    return (
        <>
            ...
            <Hero />
            <div>
                <h2 className='text-center font-bold text-4xl leading-tight'>Key Features</h2>
            </div>
            <Features />
