...
const getInvitesByUserId = async (req, res) => {
    try {
        ...
        if (!invites) {
            return res.status(httpCode.NotFound).send({ message: 'Invites not found' });
        }
        return res.status(httpCode.OK).json({ invites });
    } catch (error) {
        console.log(error);
...
const acceptInvite = async (req, res) => {
    try {
        const invite = await Invite.findById(req.params.id).populate('project user');
        if (!invite) {
            return res.status(httpCode.NotFound).json({ message: 'Invite not found' });
        }
        if (invite.user.toString() == req.user._id.toString()) {
            return res.status(httpCode.BadRequest).json({ message: 'Only user can accept invite' });
        }
        let project;
...
const rejectInvite = async (req, res) => {
    try {
        ...
        if (invite.user.toString() == req.user._id.toString()) {
            return res.status(httpCode.BadRequest).json({ message: 'Only user can reject invite' });
        }
        await invite.deleteOne();
        const invites = await Invite.find({ user: invite.user }).populate('project user');
